/* Keep track of the value of each cell in a 2d array. The game board doesn't
 * have an Update function, and instead only updates when the clock tells it to.
 *
 * Not that in UpdateBoard we look at each cell, determine what value the cell
 * should have next, then save that value to nextBoard, the secondary game
 * board. After we chack each cell, we copy all those values back.
 */

var clock     : Clock;
var nextBoard : NextGameBoard;
...
function UpdateBoard () {
  ...
  for (var i = 0; i < height; i++) {
    for (var j = 0; j < width; j++) {
      ...
      // live, time to die
      if (live && pop != 2 && pop != 3) {
        nextBoard.cells[i][j] = false;
      }
      // dead, time to live
      else if (!live && pop == 3) {
        nextBoard.cells[i][j] = true;
      }
      // otherwise don't change
      else {
        nextBoard.cells[i][j] = cells[i][j];
      }
    }
  }
  // copy the next values back to update the board
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      cells[i][j] = nextBoard.cells[i][j];
    }
  }
}
